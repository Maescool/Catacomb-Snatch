...
public class Level {
	...
	public Level(AbstractScreen screen, int width, int height) {
		...
		minimap = screen.createBitmap(width, height);
	...
	private void renderTilesAndBases(AbstractScreen screen, int x0, int y0, int x1, int y1) {
		...
		for (int y = y0; y <= y1; y++) {
			for (int x = x0; x <= x1; x++) {
				...
				AbstractBitmap[][] playerBaseZero = Art.getPlayerBase(getPlayerCharacter(0));
				AbstractBitmap[][] playerBaseOne = Art.getPlayerBase(getPlayerCharacter(1));
				...
				if (xt >= 0 && yt >= 0 && xt < baseZeroTileWidth && yt < baseZeroTileHeight && (isNotBaseRailTile(x) || yt > 0)) {
					...
					if ((xt == 4) && yt == 0) {
						screen.blit(Art.shadow_north_east, x * Tile.WIDTH + Tile.WIDTH - Art.shadow_east.getWidth(), y * Tile.HEIGHT);
					}
					continue;
				}
	...
	private void updateMinimap() {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				int i = x + y * width;
				if (hasSeen(x, y)) {
					minimap.setPixel(i, tiles[i].minimapColor);
				} else {
	...
	private void renderPanelAndMinimap(AbstractScreen screen, int x0, int y0) {
		...
		if (largeMap) {
			displaymap = calculateLargeMapDisplay(screen, x0, y0);
		} else if (smallMap) {
			displaymap = calculateSmallMapDisplay(screen);
		} else {
			displaymap = minimap;
		}
	...
	private AbstractBitmap calculateLargeMapDisplay(AbstractScreen screen, int x0, int y0) {

		AbstractBitmap largeMapDisplay = screen.createBitmap(64, 64);
		int locx = x0 + 8;
		int locy = y0 + 8;
		...
		for (int y = 0; y < 64; y++) {
			if (y < diffy || y >= (64 - diffy)) {
				for (int x = 0; x < 64; x++) {
					largeMapDisplay.setPixel(x + (y * 64), Art.floorTileColors[5 & 7][5 / 8]);
				}
			} else {
				for (int x = 0; x < 64; x++) {
					if (x < diffx || x > (64 - diffx)) {
						...
						if (((drawx + donex) + (drawy + doney) * width) < minimap.getPixelSize() - 1) {
							largeMapDisplay.setPixel(x + (y * 64), minimap.getPixel((drawx + donex) + (drawy + doney) * width));
						}
						donex++;
					}
				}
	...
	private AbstractBitmap calculateSmallMapDisplay(AbstractScreen screen) {

		AbstractBitmap smallMapDisplay = screen.createBitmap(64, 64);
		int smallx = 0, smally = 0;
		for (int y = 0; y < 64; y++) {
			for (int x = 0; x < 64; x++) {
				if (x >= (32 - width / 2) && x <= (32 + width / 2) && y >= (32 - height / 2) && y < (32 + height / 2) - 1) {
					smallMapDisplay.setPixel(x + y * 64, minimap.getPixel(smallx + smally * width));
					smallx++;
				} else {
					smallMapDisplay.setPixel(x + y * 64, Art.floorTileColors[5 & 7][5 / 8]);
				}
			}
			smallx = 0;
	...
	private void renderPlayerScores(AbstractScreen screen) {
		...
		Font.defaultFont().draw(screen, player1score, 280 - player1score.length() * 10, h - 20); //adjust so it fits in the box
		screen.blit(Art.getPlayer(getPlayerCharacter(0))[0][2], 262, h - 42);

		if (MojamComponent.instance.players[1] != null && getPlayerCharacter(1) != GameCharacter.None) {
			Font.defaultFont().draw(screen, MojamComponent.texts.scoreCharacter(getPlayerCharacter(1), player2Score * 100 / TARGET_SCORE), 56, h - 36);
			screen.blit(Art.getPlayer(getPlayerCharacter(1))[0][6], 19, h - 42);
		}
	}
