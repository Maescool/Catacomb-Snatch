...
public class AStar {
    ...
    public Path getPath(Vec2 gridStart, Vec2 gridGoal) {
        ...
        if (!canWalk(gridStart)) return new Path(false);

        Node start = createNode(gridStart);
        Node goal = createNode(gridGoal);
        if (start.pos.equals(goal.pos)) return new Path(true);
        ...
        while (queue.size() != 0) {
            ...
            for (Node neighbor : current.getNeighbors()) {
                if (neighbor.__visited) continue;
                if (!canWalk(neighbor.pos)) continue;

                double distance = current.__pathDistance + current.pos.dist(neighbor.pos);
                if (neighbor.__parent != null && distance >= neighbor.__pathDistance) continue;

                neighbor.__pathDistance = distance;
                neighbor.__heuristicDistance = neighbor.pos.dist(goal.pos) + distance;
                if (neighbor.__parent == null) {
                    neighbor.__priority = neighbor.__heuristicDistance;
                    queue.add(neighbor);
                } else neighbor.__priority = neighbor.__heuristicDistance;
    ...
    private void addNeighbors(Node n) {
        Vec2 p = n.pos;
        for (Vec2 d : dirs)
            if (canWalk(p.add(d))) n.addNeighbor(getNode(p.add(d)));
    }
