...
public class NoALPlayer implements ISoundPlayer {
	...
	private boolean playSound(String sourceName, float x, float y, boolean blocking, int index) {
		if (index < MAX_SOURCES_PER_SOUND && !isMuted() && hasWavPlaybackSupport()) {
			...
			if (!loaded.contains(indexedSourceName)) {
				getSoundSystem().newSource(false, indexedSourceName, NoALPlayer.class.getResource(sourceName), sourceName, false, x, y, 0, SoundSystemConfig.ATTENUATION_ROLLOFF, SoundSystemConfig.getDefaultRolloff());
