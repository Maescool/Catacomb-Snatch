...
>>>>>>> parent of cd61150... Cleanups, JavaDoc updates and some minor refactoring
	public DifficultySelect(boolean hosting) {
		...
		startGameButton = new Button(hosting ? TitleMenu.HOST_GAME_ID
				: TitleMenu.START_GAME_ID,
				MojamComponent.texts.getStatic("diffselect.start"),
				(MojamComponent.GAME_WIDTH - 256 - 30),
				MojamComponent.GAME_HEIGHT - 24 - 25);
		cancelButton = new Button(TitleMenu.CANCEL_JOIN_ID,
	...
	private void setupDifficultyButtons() {
		...
		for (int i = 0; i < difficulties.size(); i++) {
			...
			DifficultyCheckboxes[i] = (Checkbox) addButton(new Checkbox(i,
					difficulties.get(i).difficultyName, xStart + x * xSpacing,
					yStart + ySpacing * y));
	...
	public void render(Screen screen) {
		...
		super.render(screen);
		Font.draw(screen, MojamComponent.texts.getStatic("diffselect.title"),
				20, 20);
	}
...
>>>>>>> parent of cd61150... Cleanups, JavaDoc updates and some minor refactoring
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_LEFT
			...
			nextActiveButtonId = bestExistingDifficultyId(activeButtonId - 1,
					difficulties.size() - 1);
			...
			nextActiveButtonId = bestExistingDifficultyId(activeButtonId - 3,
		...
		if (nextActiveButtonId >= 0
				&& nextActiveButtonId < DifficultyCheckboxes.length) {
