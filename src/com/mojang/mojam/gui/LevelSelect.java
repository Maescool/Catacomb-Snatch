...
public class LevelSelect extends GuiMenu {
	...
	private final int xStart = (MojamComponent.GAME_WIDTH
			- (xSpacing * xButtons) + 8) / 2;
	...
	public LevelSelect(boolean bHosting, int localTeam) {
		...
		// Add main buttons
		startGameButton = (Button) addButton(new Button(
				bHosting ? TitleMenu.SELECT_DIFFICULTY_HOSTING_ID
						: TitleMenu.SELECT_DIFFICULTY_ID,
				MojamComponent.texts.getStatic("levelselect.start"),
				MojamComponent.GAME_WIDTH - 256 - 30,
				MojamComponent.GAME_HEIGHT - 24 - 25));
		cancelButton = (Button) addButton(new Button(TitleMenu.CANCEL_JOIN_ID,
				MojamComponent.texts.getStatic("cancel"),
				MojamComponent.GAME_WIDTH - 128 - 20,
				MojamComponent.GAME_HEIGHT - 24 - 25));
		addButton(new Button(TitleMenu.UPDATE_LEVELS,
				MojamComponent.texts.getStatic("levelselect.update"),
				MojamComponent.GAME_WIDTH - 128 - 18, 20));

		// Add page buttons
		if (levels.size() > LEVELS_PER_PAGE) {
			previousPageButton = (Button) addButton(new Button(
					TitleMenu.LEVELS_PREVIOUS_PAGE_ID, "(", xStart,
					MojamComponent.GAME_HEIGHT - 24 - 25, 30));
			nextPageButton = (Button) addButton(new Button(
					TitleMenu.LEVELS_PREVIOUS_PAGE_ID, ")", xStart + 40,
					MojamComponent.GAME_HEIGHT - 24 - 25, 30));
		}
	...
	private void updateLevelButtons() {
		...
		levelButtons = new LevelButton[Math.min(LEVELS_PER_PAGE, levels.size()
				- currentPage * LEVELS_PER_PAGE)];
		for (int i = currentPage * LEVELS_PER_PAGE; i < Math.min(
				(currentPage + 1) * LEVELS_PER_PAGE, levels.size()); i++) {
			int x = i % xButtons;
			int buttonIndex = i % LEVELS_PER_PAGE;

			levelButtons[buttonIndex] = (LevelButton) addButton(new LevelButton(
					i, levels.get(i), xStart + x * xSpacing, yStart + ySpacing
							* y, localTeam));
			if (buttonIndex == 0) {
	...
	public void render(Screen screen) {
		...
		super.render(screen);
		Font.draw(screen, MojamComponent.texts.getStatic("levelselect.title"),
				20, 20);
		...
		if (levels.size() > LEVELS_PER_PAGE) {
			if (!hasPreviousPage()) {
				...
				screen.fill(previousPageButton.getX() + 4,
						previousPageButton.getY() + 4,
						previousPageButton.getWidth() - 8,
						previousPageButton.getHeight() - 8, 0x75401f);
			}
			if (!hasNextPage()) {
				nextPageButton.render(screen);
				screen.fill(nextPageButton.getX() + 4,
						nextPageButton.getY() + 4,
						nextPageButton.getWidth() - 8,
						nextPageButton.getHeight() - 8, 0x75401f);
			}
		}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_PAGE_UP && hasPreviousPage()) {
			...
			if (e.getKeyCode() == KeyEvent.VK_LEFT
				...
				nextActiveButtonId = bestExistingLevelId(activeButtonId - 1,
						currentPage * LEVELS_PER_PAGE + 8);
				...
				nextActiveButtonId = bestExistingLevelId(activeButtonId - 3,
			...
			if (nextActiveButtonId >= 0
					&& nextActiveButtonId < levelButtons.length) {
				activeButton.setActive(false);
