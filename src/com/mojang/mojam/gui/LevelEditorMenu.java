...
public class LevelEditorMenu extends GuiMenu {
	...
	private AbstractBitmap mapFloor;// = new Bitmap(mapW, mapH);
	private AbstractBitmap minimap;// = new Bitmap(LEVEL_WIDTH, LEVEL_HEIGHT);
	private AbstractBitmap pencil;// = new Bitmap(TILE_WIDTH, TILE_HEIGHT);
	private int pencilX;
	private int pencilY;
	...
	public LevelEditorMenu(AbstractScreen screen) {
		super();
	...
	private void draw(IEditable tileOrEntity, int x, int y) {
		...
		if (mapTile[x][y] == tileOrEntity.getColor()) {
			return;
		}
	...
	private void createGUI(AbstractScreen screen) {
		mapFloor = screen.createBitmap(mapW, mapH);
		minimap = screen.createBitmap(LEVEL_WIDTH, LEVEL_HEIGHT);
		pencil = screen.createBitmap(TILE_WIDTH, TILE_HEIGHT);
