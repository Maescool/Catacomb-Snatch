...
public class AudioVideoMenu extends GuiMenu {
	...
	private Button soundVol;
	private Button musicVol;
	private Button soundsVol;
	...
	public AudioVideoMenu(boolean inGame) {
		...
		fullscreenBtn = (Button) addButton(new Checkbox(TitleMenu.FULLSCREEN_ID, MojamComponent.texts.getStatic("options.fullscreen"), xOffset, yOffset += offset, Options.getAsBoolean(Options.FULLSCREEN, Options.VALUE_FALSE)));
		fpsBtn = (Button) addButton(new Checkbox(TitleMenu.FPS_ID, MojamComponent.texts.getStatic("options.showfps"), xOffset, yOffset += offset, Options.getAsBoolean(Options.DRAW_FPS, Options.VALUE_FALSE)));
		soundVol = (Button) addButton(new Slider(TitleMenu.VOLUME, MojamComponent.texts.getStatic("options.volume"), xOffset, yOffset += offset, volume));
		musicVol = (Button) addButton(new Slider(TitleMenu.MUSIC, MojamComponent.texts.getStatic("options.music"), xOffset - xOffset / 3 - 6, yOffset += offset, musicVolume));
		soundsVol = (Button) addButton(new Slider(TitleMenu.SOUND, MojamComponent.texts.getStatic("options.sounds"), xOffset + xOffset / 3 + 6, yOffset, soundsVolume));
	...
	public void change_locale() {
		fullscreenBtn.setLabel(MojamComponent.texts.getStatic("options.fullscreen"));
		fpsBtn.setLabel(MojamComponent.texts.getStatic("options.showfps"));
		soundVol.setLabel(MojamComponent.texts.getStatic("options.volume"));
		musicVol.setLabel(MojamComponent.texts.getStatic("options.music"));
		soundsVol.setLabel(MojamComponent.texts.getStatic("options.sounds"));
