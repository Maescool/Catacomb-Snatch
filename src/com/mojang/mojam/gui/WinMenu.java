...
public class WinMenu extends GuiMenu {
	...
	public WinMenu(int gameWidth, int gameHeight, int winningPlayer) {
		...
		addButton(new Button(TitleMenu.RETURN_TO_TITLESCREEN, "Ok",
				(gameWidth - 128) / 2, 200));
	}
	...
	public void render(Screen screen) {
		...
		Font.draw(screen, MojamComponent.texts.playerWin(winningPlayer), 180,
				160);
		...
		if (winningPlayer == 1)
			screen.blit(Art.getLocalPlayerArt()[0][6],
					(gameWidth - 128) / 2 - 40, 190 + selectedItem * 40);
		if (winningPlayer == 2)
