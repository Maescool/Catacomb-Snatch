...
import com.mojang.mojam.gui.CharacterSelectMenu;
...
public class MojamComponent extends Canvas implements Runnable, MouseMotionListener,
	...
	private int localId;
	private int localTeam; // local team is the team of the client. This can be used to check if
							// something should be only rendered on one person's screen
	...
	private void init() {
		...
		if (!Options.isCharacterIDset()) {
			addMenu(new CharacterSelectMenu());
		}
	...
	private synchronized void createLevel(String levelPath, GameMode mode, int opponentCharacterID) {
		LevelInformation li = LevelInformation.getInfoForPath(levelPath);
		if (li != null) {
			createLevel(li, mode, opponentCharacterID);
			return;
	...
	private synchronized void createLevel(LevelInformation li, GameMode mode,
		...
		try {
			...
			level = mode
					.generateLevel(li, localTeam, Options.getCharacterID(), opponentCharacterID);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private synchronized void initLevel() {
		...
		players[0] = new Player(synchedKeys[0], synchedMouseButtons[0], level.width * Tile.WIDTH
				/ 2 - 16, (level.height - 5 - 1) * Tile.HEIGHT - 16, Team.Team1, localTeam,
	...
	private synchronized void render(Graphics g) {
		...
		if (player != null && menuStack.size() == 0) {
			if (isMultiplayer) {
				Font.draw(screen, texts.latency(latencyCacheReady() ? "" + avgLatency() : "-"), 10,
						20);
			}
	...
	private void tick() {
		...
		if (level != null && level.victoryConditions != null) {
			if (level.victoryConditions.isVictoryConditionAchieved()) {
				...
				int characterID = winner == localTeam ? Options.getCharacterID()
						: opponentCharacterID;
		...
		if (createServerState == 1) {
			...
			clearMenus();
			createLevel(TitleMenu.level, TitleMenu.defaultGameMode, Art.CHARACTER_HERR_SPECK);
	...
	private void tickChat() {
		...
		if (msg != null) {
			synchronizer
					.addCommand(new ChatCommand(texts.getPlayerName(Options.getCharacterID()) + ": " + msg));
		}
	}
