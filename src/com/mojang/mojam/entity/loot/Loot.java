...
public class Loot extends Entity {
    ...
    public static Bitmap[][][] anims = {
            Art.pickupCoinBronzeSmall, Art.pickupCoinSilverSmall, Art.pickupCoinGoldSmall, Art.pickupCoinBronze, Art.pickupCoinSilver, Art.pickupCoinGold, Art.pickupGemEmerald, Art.pickupGemRuby,
    ...
    public Loot(double x, double y, double xa, double ya, int val) {
        ...
        if (TurnSynchronizer.synchedRandom.nextInt(3) == 0) value++;
        if (TurnSynchronizer.synchedRandom.nextInt(3) == 0) value++;
        if (value > 8) value = 8;
...
//        value = TurnSynchronizer.synchedRandom.nextInt(9);
        ...
        animTime = TurnSynchronizer.synchedRandom.nextInt(anims[value].length * 3);
    ...
    public void tick() {
        ...
        if (isTakeable) {
            ...
            for (Entity e : level.getEntities(getBB().grow(dist))) {
                if (!(e instanceof LootCollector)) continue;
                ...
                if (!p.canTake()) {
                    ...
                    if (xd * xd + yd * yd < localDist * localDist) {
                        ...
                        xd /= dd;
                        yd /= dd;
                        double pow = (1 - (dd / localDist)) * 1.6 * (suckPow * 0.5 + 0.5);
                        if (z <= 0) {
                            this.xa += xd * pow;
                            this.ya += yd * pow;
                        }
    ...
    protected void onTake(LootCollector taker) {
        ...
        if (value > 8) {
            ...
            MojamComponent.soundPlayer.playSound("/sound/Big Coin.wav", (float) pos.x, (float) pos.y);
        } else {
            MojamComponent.soundPlayer.playSound("/sound/Small Coin.wav", (float) pos.x, (float) pos.y);
        }
    }
