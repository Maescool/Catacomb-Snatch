...
public abstract class HostileMob extends Mob {
	...
	public int FaceEntity(double x, double y, double radius,
			Class<? extends Entity> c, int facing) {
		Set<Entity> entities = level.getEntities(pos.x - radius,
				pos.y - radius, pos.x + radius, pos.y + radius, c);
		Entity closest = null;
		...
		if (closest != null
				&& !isTargetBehindWall(closest.pos.x, closest.pos.y, closest)) {
			chasing = true;
			double angle = Math.atan2((closest.pos.y - pos.y),
					(closest.pos.x - pos.x));
	...
	public void tick() {
		super.tick();
		Tile thisTile = level.getTile((int) pos.x / Tile.WIDTH, (int) pos.y
				/ Tile.HEIGHT);
	...
	public void collide(Entity entity, double xa, double ya) {
		if (entity instanceof Mob) {
			...
			if (isNotFriendOf(mob)) {
				mob.hurt(this,
						DifficultyInformation.calculateStrength(strength));
			}
		}
	}
