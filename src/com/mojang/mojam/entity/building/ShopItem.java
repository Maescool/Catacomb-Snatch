...
public abstract class ShopItem extends Building {
	...
	public ShopItem(String name, double x, double y, int team, int cost,
		int yOffset) {
		super(x, y, team);
		this.name = name;
	...
	protected void renderInfo(AbstractScreen screen) {
		...
		if (highlight) {
			...
			int width = getLongestWidth(screen, tooltip, Font.FONT_WHITE_SMALL) + 4;
			int height = tooltip.length
	...
	private int getLongestWidth(AbstractScreen screen, String[] string, Font font) {
		int res = 0;
		for (String s : string) {
			int w = font.calculateStringWidth(screen, s.trim());
			res = w > res ? w : res;
		}
		return res;
	}
			...
			if (!player.isCarrying() && canBuy(player)
				...
				if (this.team == MojamComponent.localTeam) {
					Notifications.getInstance().add(
						MojamComponent.texts.upgradeNotEnoughMoney(effectiveCost));
				}
